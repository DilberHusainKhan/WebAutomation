...
async function getDetailsFromIndeed(page) {
    ...
    await page.waitForSelector('a[href="/jobs?q=Software+Developer&l=Gurgaon&start=10"]');
    await page.click('a[href="/jobs?q=Software+Developer&l=Gurgaon&start=10"]');
...
async function searchForKeyWordsInIndeed(page) {
    ...
    await page.type('input[name="q"]', data.Keywords.Role);
    await page.type('input[name="l"]', data.Keywords.location);
...
async function typeKeyValuesIntoNaukri(page) {
    ...
    await page.type('input[name="qp"]', data.Keywords.Role);
    await page.waitForSelector('input[name="ql"]');
    await page.type('input[name="ql"]', data.Keywords.location);
...
async function typeKeyValuesIntoLinkedIn(page) {
    ...
    await page.type('input[name="keywords"]', data.Keywords.Role);
    await page.waitForSelector('input[name="location"]');
    await page.type('input[name="location"]', data.Keywords.location);
    for (let i = 0; i < 30; i++) {
